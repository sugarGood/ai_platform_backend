...
/**
 * 故障转移策略管理控制器。
 * <p>提供 {@code /api/admin/failover-policies} 下的 REST 端点，支持策略创建和查询操作。</p>
 */
@RestController
...
public class ProviderFailoverPolicyController {
     ...
     * 构造方法，注入策略业务服务。
     *
     * @param providerFailoverPolicyService 策略服务
     */
    ...
    public ProviderFailoverPolicyResponse create(@Valid @RequestBody CreateProviderFailoverPolicyRequest request) {
    ...
    public List<ProviderFailoverPolicyResponse> list() {
        return providerFailoverPolicyService.list().stream()
                .map(ProviderFailoverPolicyResponse::from)
                .toList();
    }
